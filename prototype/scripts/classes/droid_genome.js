...
class Droid_Genome{ 
	...
	createPhenotype(genesDecoded=null, needSpecOnly=false){
		...
		var phenotype = {
			...
			"servo_leg_left" : { 
				"angleRange" : { "setType": "variable-range", "min": 0, "max": 0.785, "set" : genesDecoded[0] }, 
			...
			"servo_leg_right" : { 
				"angleRange" : { "setType": "variable-range", "min": 0, "max": 0.785, "set" : genesDecoded[4] }, 
			...
			"servo_foot_left" : { 
				"angleRange" : { "setType": "variable-range", "min": 0, "max": 0.785, "set" : genesDecoded[8] }, 
			...
			"servo_foot_right" : { 
				"angleRange" : { "setType": "variable-range", "min": 0, "max": 0.785, "set" : genesDecoded[12] }, 
		...
		let crazy_number = mulberry32(joint_legGap_seed);
		let joint_legGap = map(crazy_number, [0.0, 1.0], [0.1, genesDecoded[16]/2.5]);
	...
	encodeGenes(baseGenes, _geneSpec){
		...
		// Gene 20 (leg gap) is now handled by a genome derivative
	...
	decodeGenes(encodedGenes, _geneSpec){
		...
		function geneDecode(baseValue, randomType, range){
			...
			if(randomType=="option"){ 
				...
				let choiceIndex = map(baseValue, [0.0, 1.0], [0, range.length-1]);
